...
'''
Error message to display whem this script is called with one too
many argument or less
'''

class monitor:
    '''
    Set variables that will be used throughout the class
    '''
    ...
    def __init__(self, url):
        '''
        Read the URL of interest and set variables that depend on its value
        '''
        self.url = url
        ...
        for j in self.soup.find_all(self.script):
            ...
            if not jsrc == None:
                ...
                it is missing and raise a system exit on connection error
                '''
                try:
                    if self.slash*2 in jsrc:
                        p = requests.get(jsrc)
                    else:
                        p = requests.get(url + jsrc if url[-1] ==
                            self.slash else url + self.slash + jsrc)
                except requests.exceptions.RequestException as e:
                    raise SystemExit(e)
    ...
    def match(self):
        '''
        Create a directory named by the sanitised URL under the directory
        of the script before checking the downloaded contents for update
        '''
        try:
        ...
        '''
        Call the __write method to output the SHA 256-bit checksum and
        a copy of the downloaded contents if a previous SHA 256-bit
        checksum file is not found
        '''
        try:
            with open(self.wd + self.hname ,'r') as f:
                '''
                Call the __email method to send an email notification
                if the SHA 256-bit checksum does not match a previous
                one and the __write method to overwrite the SHA 256-bit
                checksum file with the latest one and output a copy of
    ...
    def __write(self):
        '''
        Output the SHA 256-bit checksum and the rest of the downloaded
        contents by overwriting existing ones or creating new files
        '''
        try:
...
if __name__ == '__main__':
    if len(argv) == 2:
        ...
        method for matching if one and precisely one argument is
        supplied and raise a system exit if otherwise
        '''
        w = monitor(argv[1])
        w.match()
    else:
